...
public class MyList<Data>
{
    ...
    public MyList<Data> rearrange() {
        MyList<Data> list1=new MyList<>();
        MyList<Data> list2=new MyList<>();
        MyList<Data> list3=new MyList<>();
        Node n=head;
        while (n!= null) {

            if ((Integer)n.value <=0)
                list1.push(n.value);
            else
                list2.push(n.value);
            n = n.next;
        }
        Data list1Min=list1.getValue();
        Data list2Min=list2.getValue();
        ...
        while(true) {
            for (int i = 0; i < size(); i++) {
                ...
                {
                    list1.setToBegin();
                    if((Integer)list1Min > (Integer)list1.getValue())
                    {
                        list1Min=list1.getValue();
                        list1.erase();
                        list1.next();
                        list1Min=list1.getValue();
                        list3.push(list1Min);
                ...
                {
                    list2.setToBegin();
                    if((Integer)list2Min > (Integer)list2.getValue())
                    {
                        list2Min=list2.getValue();
                        list2.erase();
                        list2.next();
                        list2Min=list2.getValue();
                        list3.push(list2Min);
